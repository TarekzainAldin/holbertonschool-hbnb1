...
class TestDataManager(unittest.TestCase):
    ...
    def test_save_review(self, mock_save_review):
        ...
        review_data = {'comment': 'Great place!', 'user_id': 1, 'place_id': 1, 'rating': 5}
    ...
    def test_get_review(self, mock_get_review):
        mock_review = Review(comment='Great place!', user_id=1, place_id=1, rating=5)
    ...
    def test_get_all_reviews(self, mock_get_all_reviews):
        mock_reviews = [
            Review(comment='Great place!', user_id=1, place_id=1, rating=5),
            Review(comment='Amazing!', user_id=2, place_id=2, rating=4)
        ]
        mock_get_all_reviews.return_value = mock_reviews
        result = self.data_manager.get_all_reviews()
        self.assertEqual(len(result), 2)
        self.assertEqual(result[0].comment, 'Great place!')
        self.assertEqual(result[1].comment, 'Amazing!')
    ...
    def test_update_review(self, mock_update_review):
        ...
        result = self.data_manager.update_review(1, updated_data)
        self.assertTrue(result)
        mock_update_review.assert_called_once_with(1, updated_data)

    @patch.object(DataManager, 'delete_review')
    def test_delete_review(self, mock_delete_review):
        mock_delete_review.return_value = True
        result = self.data_manager.delete_review(1)
        self.assertTrue(result)
        mock_delete_review.assert_called_once_with(1)
